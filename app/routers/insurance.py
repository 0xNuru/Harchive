...
@router.post(
    "/admin/register",
    response_model=insuranceSchema.ShowInsurance,
...
def create_in_admin(request: insuranceSchema.InAdmin, db: Session = Depends(load)):
    ...
    checkPhone = (
        db.query_eng(insuranceModel.InAdmin)
        .filter(insuranceModel.InAdmin.phone == phone)
        .first()
    )
    checkInsuranceID = (
        db.query_eng(insuranceModel.InAdmin)
        .filter(insuranceModel.InAdmin.insuranceID == insuranceID)
        .first()
    )
...
@router.get(
    "/admin/all",
    response_model=List[insuranceSchema.ShowInsurance],
...
@router.get(
    "/admin/insuranceID/{insuranceID}",
    response_model=insuranceSchema.ShowInsurance,
    status_code=status.HTTP_200_OK,
)
def show_admin(
    insuranceID, db: Session = Depends(load), user_data: get_current_user = Depends()
):
    roles = ["insurance_admin"]
    check_role(roles, user_data["user_id"])
    admin = (
...
def create_insurance(
    request: insuranceSchema.Insurance,
    ...
    insuranceID = request.insuranceID
    checkPhone = (
        db.query_eng(insuranceModel.Insurance)
        .filter(insuranceModel.Insurance.phone == phone)
        .first()
    )
    checkInsuranceID = (
        db.query_eng(insuranceModel.Insurance)
        .filter(insuranceModel.Insurance.insuranceID == insuranceID)
        .first()
    )
