...
@router.post(
    "/register",
    response_model=patientSchema.ShowPatient,
...
def create_patient(request: patientSchema.Patient, db: Session = Depends(load)):
    ...
    checkPhone = (
        db.query_eng(userModel.Users).filter(userModel.Users.phone == phone).first()
    )
    checkEmail = (
        db.query_eng(userModel.Users).filter(userModel.Users.email == email).first()
    )
    if checkPhone:
        raise HTTPException(
    ...
    new_patient = patientModel.Patient(
        name=request.name,
        phone=request.phone,
        email=request.email,
...
def show(
    ...
    check_role(roles, user_data["user_id"])
    patient = (
        db.query_eng(patientModel.Patient)
        .filter(patientModel.Patient.email == email)
        .first()
    )
    if not patient:
        raise HTTPException(
...
@router.post(
    "/record/add",
    response_model=patientSchema.PatientRecord,
    status_code=status.HTTP_201_CREATED,
)
def create_patient_record(
    request: patientSchema.PatientRecord,
    user_data: get_current_user = Depends(),
    db: Session = Depends(load),
):
    ...
    check = (
        db.query_eng(recordModel.Record)
        .filter(recordModel.Record.patient == id)
        .first()
    )
    if check:
        raise HTTPException(
...
@router.get(
    "/record/all",
    response_model=List[patientSchema.PatientRecord],
    status_code=status.HTTP_200_OK,
)
...
def show(
    nin: int, user_data: get_current_user = Depends(), db: Session = Depends(load)
):
    roles = ["patient", "hospital_admin", "doctor"]
    check_role(roles, user_data["user_id"])
    patient = (
        db.query_eng(patientModel.Patient)
        .filter(patientModel.Patient.nin == nin)
        .first()
    )
    if not patient:
        raise HTTPException(
...
def update_admin(
    ...
    updated_record = recordModel.Record(
        type=request.type,
        patient=id,
        DOB=request.DOB,
        BloodType=request.BloodType,
...
@router.post(
    "/medication/add/{nin}",
    response_model=patientSchema.ShowMedication,
    status_code=status.HTTP_201_CREATED,
)
def create_patient_medication(
    nin: int,
    request: patientSchema.Medication,
    user_data: get_current_user = Depends(),
    db: Session = Depends(load),
):
    roles = ["doctor"]
    ...
    doctor = (
        db.query_eng(hospitalModel.Doctors)
        .filter(hospitalModel.Doctors.id == id)
        .first()
    )
    if not patient:
        raise HTTPException(
    ...
    new_medication = patientModel.Medication(
...
@router.post(
    "/allergy/add/{nin}",
    response_model=patientSchema.ShowAllergy,
    status_code=status.HTTP_201_CREATED,
)
def create_patient_allergy(
    nin: int,
    request: patientSchema.Allergy,
    user_data: get_current_user = Depends(),
    db: Session = Depends(load),
):
    ...
    new_allergy = patientModel.Allergy(
...
@router.post(
    "/immunization/add/{nin}",
    response_model=patientSchema.ShowImmunization,
    status_code=status.HTTP_201_CREATED,
)
def create_patient_immunization(
    ...
    new_immunization = patientModel.Immunization(
        patient=patient.id,
        name=request.name,
        hospitalID=doctor.hospitalID,
...
@router.post(
    "/transaction/add/{nin}",
    response_model=patientSchema.Transaction,
    status_code=status.HTTP_201_CREATED,
)
def create_patient_transaction(
    nin: int,
    ...
    new_transaction = patientModel.Transactions(
        patient=patient.id,
        hospitalID=doctor.hospitalID,
        doctor_id=id,
        doctor_name=user_data["username"],
