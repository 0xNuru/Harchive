...
@router.post("/register", response_model=userSchema.ShowUser, status_code=status.HTTP_201_CREATED)
def create_user(request: userSchema.User, db: Session = Depends(load)):
    phone = request.phone
    email = request.email

    checkPhone = (db.query_eng(userModel.Users).filter(userModel.Users.phone == phone).first())
    checkEmail = (db.query_eng(userModel.Users).filter(userModel.Users.email == email).first())
    if checkPhone:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=f"user with phone: {phone} exists")
    if checkEmail:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT,detail=f"user with email: {email} exists")
...
@router.get(
    "/all", response_model=List[userSchema.ShowUser], status_code=status.HTTP_200_OK
)
...
@router.get(
    "/email/{email}", response_model=userSchema.ShowUser, status_code=status.HTTP_200_OK
)
def show(email, db: Session = Depends(load)):
    users = db.query_eng(userModel.Users).filter(userModel.Users.email == email).first()
    if not users:
        raise HTTPException(
...
def login(
    response: Response,
    request: userSchema.UserLogin = Depends(),
    ...
    check = db.query_eng(userModel.Users).filter(userModel.Users.email == email).first()
    if not check:
        raise HTTPException(
    ...
    patient = (
        ...
        .filter(patientModel.Patient.id == check.id)
        .first()
    )
...
async def refresh(
    ...
    try:
        ...
        check = (
            db.query_eng(userModel.Users)
            .filter(userModel.Users.email == user_email)
            .first()
        )
        ...
        if error == "MissingTokenError":
            ...
            return JSONResponse(
                content={"redirect_url": redirect_url, "redirect": True},
                status_code=307,
            )
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=error)

    data = {"username": check.name, "email": check.email, "user_id": check.id}
